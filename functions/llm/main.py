...
async def llm(body: LlmBody, test=False):
    ...
    tasks = [
        get_response(
            args,
            body.options.stream,
            body.options.maxCost * max_cost_pcts[i],
            i if test else None,
        )
...
async def get_response(args: LlmArgs, stream: bool, maxCost: float, test=None):
    ...
    if args.max_completion_tokens is None:
        ...
        raise HTTPException(
            status_code=400,
            detail="max_completion_tokens must be less than 99000 when streaming is disabled",
        )
    model, expected_cost = find_model(
        args, maxCost
    )  # note that this may modify args.messages and args.max_completion_tokens
    ...
    default_reasoning_effort = supported_models[model].get(
        "default_reasoning_effort", None
    )
    if args.reasoning_effort is None and default_reasoning_effort is not None:
    ...
    if args.reasoning_effort is not None:
        # todo should be able to remove this once litellm supports gpt-5
        api_args.update(
            {
...
def find_model(args: LlmArgs, maxCost: float):
    ...
    for model in models:
        ...
        if input_tokens is None:
            input_tokens = messages_token_counter(
                args.messages, tokenizer, model_info["max_vision_tokens"]
            )
        ...
        if expected_cost <= maxCost:
            ...
            logger.debug(
                "expected_cost %s > maxCost %s for model %s: input_tokens %s, output_tokens %s",
                expected_cost,
                maxCost,
                model,
                input_tokens,
...
def messages_token_counter(messages, tokenizer, max_vision_tokens, return_list=False):
    ...
    for message in messages:
        messages_token_counts.append(
            message_token_counter(message, tokenizer, max_vision_tokens)
        )
    if return_list:
        ...
        if isinstance(value, str):
            ...
            for c in value:
                if c["type"] == "text":
                    ...
                    raise HTTPException(
                        status_code=400, detail=f"Unknown message type: {c['type']}"
                    )
        else:
...
def trim_messages_for_cost(args: LlmArgs, model_info, input_tokens, maxCost):
    ...
    if input_cost + output_cost <= maxCost:
        return args.messages  # we're done without modifying messages
    input_token_budget = (maxCost - output_cost) / model_info[
...
def trim_messages_impl(args, model_info, input_token_budget):
    ...
    if args.messages[0]["role"] == "system" or args.messages[0]["role"] == "developer":
        ...
        forward_budget = min(
            messages_token_counts[0][1] + floor(input_token_budget * 0.2),
            floor(input_token_budget * 0.8),
        )
    else:
        forward_budget = floor(input_token_budget * 0.2)
    ...
    messages_backward_budget = trim_message_loop(
        messages_token_counts, backward_budget, forward=False
    )
...
def trim_message(
    ...
    if isinstance(content, str):
        ...
        ]  # todo should just do this earlier so we don't have to handle strings throughout the code
    # handle images
    ...
    for i, c in enumerate(content):
        ...
        if c["type"] == "text":
            if found_text:
                ...
                continue
            found_text = True
            forward_budget = floor(
                content_budget * forward_budget / (forward_budget + backward_budget)
            )
    ...
    message["content"] = (
        content  # in case it was a string, we didn't mutate the original
    )
    return message
...
def handle_stream_response(results):
    ...
    return StreamingResponse(
        length_prefix_transform(handle_stream_final_cost(stream), final_object=True),
...
async def handle_stream_result(result, index=None):
    ...
    if first_chunk:
        yield data(
            model=model, content=buffer, finish_reason=finish_reason, usage=usage
        )
    else:
