...
model picker
drag and drop
feedback - not possible to run inside devlocal
reload
*/
...
test.describe(() => {
  ...
  test("Assistant", async ({ app }) => {
    ...
    await expect(
      app.getByRole("dialog").getByRole("button").nth(1),
    ).toBeVisible({
      timeout: 20000,
    });
      ...
      window.requestFunction = async (fn, args, options) => {
        if (fn.startsWith("magicsandbox.llm")) {
          ...
          if (lastMessageContent.includes("Hello1")) {
            ...
            if (
              /<logs>\s*\S[\s\S]*<\/logs>/.test(lastMessageContent) === false
            ) {
...
test.describe(() => {
  test.use({ appOptions: { autoConfirm: true, autoInit: false } });

  test("tutorial", async ({ app }) => {
    await app.evaluate(async () => {
      window._TESTING = {
        seenTutorial: false,
        initApp: "magicsandbox.Notes",
      };
      await app.init();
    });
    expect(app.getByText("Click to start tutorial")).toBeVisible();
  });
});
