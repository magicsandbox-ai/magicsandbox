...
function diffExtension(
  ...
  const diffHandler = EditorState.transactionFilter.of((tr) => {
    try {
      ...
      //check if we should deactivate the changeSet - if there is exactly one chunk that the user is accepting/rejecting
      if (
      ...
      //apply the transaction to the original document by mapping it through the changeSet
