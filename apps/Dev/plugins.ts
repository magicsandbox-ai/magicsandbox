...
function getImport(node: Node) {
  ...
  if (node.type === "ImportDeclaration") {
    ...
    return { path: node.source.value as string, names };
 ...
 */
function getImports(file: string) {
...
*/
function transformImports(file: string, pkgImports: PkgImports) {
  const out: string[] = [];
...
^Actually, this is not true (I'm not sure why I thought so). So could try reexporting again
*/
function transformToBundleDeps(pkgImports: PkgImports) {
  ...
  const outputAssignments: string[] = [];
    ...
    setup(build: Esbuild.PluginBuild) {
      ...
      build.onResolve({ filter: /.*/ }, async (args) => {
        ...
        if (readFile(path) !== undefined) {
          ...
          path.startsWith("./") &&
          readFile(path.slice(2)) !== undefined
        ) {
...
async function bundleDeps(
  ...
  if (appObj.optimizedTreeShaking !== false) {
    ...
    if (result.resolvedPaths) {
      ...
      console.warn(
        `optimizedTreeShaking is not enabled for ${appObj.cdn}`, //todo show user
      );
    }
  }
...
function getAppObj(readFile: ReadFile) {
  const appString = readFile("magic.json");
...
class BuildMetadata {
  ...
  importQueue: Import[];
  packageMetadataMap: Record<string, PackageMetadata>;
...
class PackageMetadata {
  ...
  constructor(buildMetadata: BuildMetadata, packageId: string) {
...
class Import {
  ...
  constructor(
    ...
    this.onResolvePromise.catch((error: Error) => {
  ...
  handleResolve() {
    if (this.resolvedPath === undefined) {
      throw new Error(`Unexpected error resolving path ${this.args.path}`);
    }
    try {
 ...
 */
function parseNormalizedPath(path: string) {
  const match = path.match(
    // scope         package  version      file
    /^(?:@([^/]+)\/)?([^@/]+)?(?:@([^/]+))?(?:\/(.+))?$/,
  );
  if (match === null) {
    throw new Error(`Invalid normalized path: ${path}`);
  }
  return {
    scope: match[1],
    package: match[2],
...
function normalizePath(
  ...
  const readFile = buildMetadata.readFile;
  if (readFile(path) !== undefined) {
    return `/${path}`;
  } else if (
    path.startsWith("./") &&
...
function getUrls(resolvedPath: string, buildMetadata: BuildMetadata) {
